...
CChartAxis::CChartAxis(CChartView* pParent,bool bHoriz):CChartObject(pParent)
{
...
void CChartAxis::SetDateTimeFormat(bool bAutomatic, CString strFormat)
{
...
void CChartAxis::Draw(CDC *pDC)
{
	...
	NewFont.CreatePointFont(m_nFontSize,CString(m_strFontName),pDC);
...
void CChartAxis::DrawLabel(CDC* pDC)
{
	...
	if (m_bIsHorizontal)
	{
		if (!m_bIsSecondary)
		{
			...
			YPos = m_ObjectRect.top  + TextSize.cy + 2;           //改动 设置X轴的标签位置
	...
	else
	{
		if (!m_bIsSecondary)
		{
			YPos = HalfAxisPos + LabelSize.cy/2;                 //改动 设置Y轴的标签位置
			XPos = m_ObjectRect.left + 0;
		}
		else
		{
			YPos = HalfAxisPos + LabelSize.cy/2; 
			XPos = m_ObjectRect.right - LabelSize.cx - 370;          //改动 设置Z轴的标签位置
		}
	}
	m_pAxisLabel->SetPosition(XPos,YPos,pDC);       //设置位置  改动
...
{
    long Offset = 0;
	if (dbIsEqu(m_MaxValue,m_MinValue))
	{
		Offset = (long)fabs((m_EndPos-m_StartPos)/2.0);
		if (m_bIsHorizontal)
	...
	if (m_AxisType != atLogarithmic)
        Offset = (long)floor( (Value - m_MinValue) * GetAxisLenght()/(m_MaxValue-m_MinValue) );
	else
		Offset = (long)floor((log10(Value)-log10(m_MinValue)) * GetAxisLenght()/(log10(m_MaxValue)-log10(m_MinValue)) );
...
void CChartAxis::SetFont(int nPointSize,CString  strFaceName)
{
...
bool CChartAxis::RefreshAutoAxis()
{
	...
	if ( !dbIsEqu(SeriesMax,m_MaxValue) || !dbIsEqu(SeriesMin,m_MinValue) )
...
void CChartAxis::CalculateFirstTick()
{
	...
	case atStandard:
		{
			m_FirstTickVal = 0;
			if (!dbIsEqu(m_TickIncrement,0))
			{
				if (dbIsEqu(m_MinValue , 0))
					m_FirstTickVal = 0;
				...
				else
				{
					...
					while (m_FirstTickVal>m_MinValue)
						m_FirstTickVal -= m_TickIncrement;
					if (!dbIsEqu(m_FirstTickVal , m_MinValue))
						m_FirstTickVal += m_TickIncrement;
				}
			}
	...
	case atLogarithmic:
		{
			...
			if (LogDecCount<0)
				LogDecCount = -(LogDecCount);
			else
				LogDecCount = 0;
			strLabel.Format(_T("%.*f"),LogDecCount,TickValue);
		}
		break;
	...
	case atDateTime:
		{
			...
			strLabel = tickTime.Format(m_strDTTickFormat);
		}
		break;
	}
...
void CChartAxis::ShowScrollBar(bool bShow)
{
	if (m_pScrollBar)
	{
		if (bShow)
