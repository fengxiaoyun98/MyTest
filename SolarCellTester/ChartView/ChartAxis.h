...
public:
	...
	void SetDateTimeFormat(bool bAutomatic, CString strFormat);
	...
	CChartAxis(CChartView* pParent,bool bHoriz);
	virtual ~CChartAxis();
	...
	{ 
		if (m_pScrollBar)
			return (m_pScrollBar->IsWindowVisible()!=0); 
		else
			return false;
	}

private:
	bool dbIsEqu(double a,double b)const{return fabs(a-b)<DBL_EPSILON;}
	...
	SeriesList m_pRelatedSeries;	//此用来计算自动坐标MinMax值 List containing pointers to series related to this axis
	...
	CString m_strDTTickFormat;		// Format of the date/time tick labels
