 ...
 *
 *	History:
 *		- 02/03/2008: Legend can now be docked on any side or can be doubleing.
...
CChartLegend::CChartLegend(CChartView* pParent):CChartObject(pParent)
{
...
void CChartLegend::SetFont(int iPointSize,CString strFaceName)
{
...
void CChartLegend::UpdatePosition(CDC* pDC, const CRect& rcControl)
{
	...
	for (size_t i=0;i<SeriesCount;i++)
	{
		...
		Drawn++;
		TextSize = pDC->GetTextExtent(pSerie->GetName());
...
void CChartLegend::Draw(CDC *pDC)
{
	...
	for (unsigned int i=0;i<SeriesCount;i++)
	{
		...
		int MaxHeight = 0;
		CSize TextSize = pDC->GetTextExtent(pSerie->GetName());
		if (TextSize.cy > m_BitmapSize.cy)
		{
			pDC->ExtTextOut(rectBitmap.right+4,rectBitmap.top,ETO_CLIPPED,NULL,pSerie->GetName(),NULL);
		...
		else
		{
			...
			pDC->ExtTextOut(rectBitmap.right+4,YOffset,ETO_CLIPPED,NULL,pSerie->GetName(),NULL);
